...
def count_words(phrase):
    ...
    #iterate words in phrase to count and update dictionary
    for word in phrase: 
...
def get_melon_price(melon_name):
    ...
    prices = melon_dict.values()

    melon_names = melon_dict.keys() 

    if melon_name not in melon_dict.keys(): 
...
def word_length_sorted(words):
    ...
    # want to make dictionary where 
    # keys = word length, and, 
    # values = words with that word length

    #get words in alphabetical order
    words = sorted(words)

    for word in words: 
        if word not in uniq_set: 
            uniq_set.add(word)
            key = len(word)
...
def kids_game(names):
    ...
    results[0][-1] == # next letter begins with this 
